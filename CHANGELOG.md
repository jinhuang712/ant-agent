...
派它盯 CI，它回了 `Verdict: success`，没说盯的是哪一条 run——我得自己去列表里翻。

模板里原本有个 `Link:` 字段，但写成了「可点击页面，没有就填 none」，于是没有网页可给的时候坐标就整个丢了。共同契约要求每条结论带 pin，这个模板没有承接它的位置。

改成 `Pin:` 必填，并写明**成功时也要给**——绿的结论跟红的一样要被核，而调用方找不到就核不了。

### 等待时长和运行时长是两回事

它同一次报的 `Elapsed: 2`，而那条 run 实际跑了 10 秒。两个数都没错：它开始轮询时 run 已经跑了一会儿了。但只报一个数、还叫 elapsed，读的人会以为 CI 两秒就完事。

拆成 `Waited`（你盯了多久）和 `Ran`（它自己跑了多久，读不到就写 unknown）。

**这两条都是它自己暴露的**：它在 `Unplanned` 里标了「2 秒 vs 历史 11–16 秒，可能有静默跳步，建议核日志」。结论对、数字有歧义、而歧义是它自己指出来的。

## 0.3.6 — 2026-08-24

### CI 从破坏矩阵进去那天起就是红的

`scripts/breakage-matrix.sh` 里的仓库根写死成了我本机的绝对路径。CI runner 上没有那个路径，前置检查直接 ABORT，退出码 1。

```
ABORT /Users/jin.huang/dev/skills/ant-agent 不是 git 工作区
```

改成从脚本自身位置推导。

**连红四次没人发现**，因为每次我都只看本地跑的结果，把「本地全绿」当成了「CI 全绿」报出去。加一个 CI 步骤却不看它在 CI 里的第一次运行，等于没加——这个矩阵存在的理由就是「校验跑过了不等于校验有用」，而它自己犯的正是同一类错。

## 0.3.5 — 2026-08-21

### pardon 说了自己是什么，没说什么时候该选它

八只里 `ant:pardon` 是唯一从未被派过的——跨多个 session，一次都没有。我一直归因于它的准入判据严（过程确定 · 难搞错 · 可事后验 · 不可逆的不派），六轮自测每轮都绕开。

重读之后发现不是判据的事。它的 description 只写了：

```
It differs from ant-adjust on exactly one axis — whether the result comes back.
```

这句讲清了**差异**，没讲**理由**。调用方读完会卡在「那我为什么要不看结果」上——不看的好处是零，看一眼的成本极低，于是永远选 adjust。一个说不出自己何时该被选中的角色，等于不可认领。

真正的理由是**规模**：批量大到结果本身没有阅读价值。五条结果读起来很便宜，还能读出东西；四十条不行。补进 description 了，`thought` 句也从「just do it」换成带出规模的那句。

这个仓产生不了那种规模的活，所以六轮都撞不上——处境是真的，只是不在这里发生。

## 0.3.4 — 2026-08-21

### 探针从 16 个补到 29 个，覆盖全部 28 个 fail 落点

上一版说「16/16 拦下」，那句话真正的意思是十六个探针都被拦住了，不是十六条校验都验过了。`validate.mjs` 有 **28 个 `fail()` 落点**，之前只有一半左右被打到过。

现在每个落点都至少被一个探针触发一次。

### 更要紧的是判定方式变了

原来的探针只看 `validate.mjs` 退出码非 0 就算 PASS。漏洞在于：一个破坏可能顺带触发**别的**校验，退出码照样非零，探针照样报 PASS——而它声称要测的那条压根没被打到，没人会发现。

现在每个探针要额外声明**预期错误特征**，比对实际输出里有没有那句话。多出一种状态：

| 状态 | 含义 |
|---|---|
| PASS | 退出码非零，且报的正是这条探针要测的错 |
| MISS | 破坏了但校验放行——校验有洞 |
| WRONG | 退出码非零但报的是别的错——**探针打偏了**，不是校验失效 |

回填到全部 16 个老探针之后重跑，没有一条暴露出打偏。

### 缺口清单是我给漏的

我列了 10 处没覆盖的位置交给 `ant:adjust`。它自己把 28 个落点数了一遍，另外发现 3 处我没列的：行 134（`Claude 侧缺产物`，133 的镜像方向）、行 135（`没有任何产物`）、行 179（`Codex plugin.json 不存在`，175 的镜像）。三处一起补了。

本地实跑：拦下 29 / 放行 0 / 打偏 0，退出码 0，工作区还原干净。

## 0.3.3 — 2026-08-21

### 校验有没有效，现在也归 CI 管

`scripts/breakage-matrix.sh` 进仓，挂在 `validate` 之后。它逐条破坏 `validate.mjs` 的每一条校验，确认真的拦得住；任意一条被放行就让 CI 红。

在此之前 CI 只保证「校验跑过了」，不保证「校验还有用」。一条写坏的校验永远返回通过，前后两种情况在 CI 里长得一模一样。

16 条全部有探针，本地跑 16/16 拦下。

### 一个跟本项目无关但值得记的 bash bug

改造过程中原脚本的日志输出是乱码——`PASS $name（退出码 $code）` 打成了 `PASS ��退出码 ��`，变量和全角括号一起没了。

我判定的根因是「`nohup` 没继承 `LANG`」，写进任务书里当既定事实交给了 `ant:adjust`。**它顶回来了**，隔离出真因：

```
bash 5.3.15  有 LANG → ��退出码 ��
bash 5.3.15  无 LANG → ��退出码 ��      ← locale 不影响
${n} 花括号          → 测试名（退出码 1）
printf -v            → 测试名（退出码 1）
bash 3.2             → 测试名（退出码 1）  ← 老版本没这个 bug
```

真因是 **bash 5.3 的 word expansion**：裸 `$var` 紧跟多字节字面量（中间没有 ASCII 字节）时，变量内容被丢弃、字面量首字节被截断。跟 locale 无关。`${var}` 或 `printf -v` 都能绕开。

我那次对照实验拿外层 zsh 跟 `bash -c` 比，以为只改了 `LANG`，其实同时换了 shell。

### 顺带修的两处

- **那条唯一 MISS 的测例本身是错的**：往源卡 `situation` 里塞冒号指望 YAML 校验拦下，但生成器把它折成 `>-` 块标量，块标量内冒号本就合法。改成打 `color:`——生成产物里唯一保留裸标量的字段
- **两个 probe 把版本号写死成 `0.3.1`**，仓库已经到 0.3.2，sed 匹配不上会静默 no-op 然后误报 MISS。改成正则
