...
**多只并行就等到齐。** 扫了七个用过蚂蚁的 session，最普遍的缺陷不是话术而是节奏：一件事被拆成派发公告、等待汇报、结果分批到达好几段输出，六个 session 全中约 24 次。中间那些「某条证据链已经闭合，等另一只回来」占了用户一次阅读却不含新信息。根因是后台蚂蚁完成会唤醒主会话，而被唤醒就想说话——**一只先回不是说话的理由**。
